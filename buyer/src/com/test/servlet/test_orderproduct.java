...
/**
 * Servlet implementation class test_orderproduct
 */
@WebServlet("/test_orderproduct")
public class test_orderproduct extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public test_orderproduct() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(buyer==null){
			...
			out.print("<html><head>"
					+ "<script>"
					+ "alert('please login');parent.location.replace('/buyer/test_showproduct');"
					+ "</script>"
			...
			out.print("<html><head>"
					+ "<script>"
					+ "alert('something wrong');parent.location.replace('/buyer/test_showproduct');"
					+ "</script>"
					+ "<meta http-equiv='Content-Type' content='text/html; charset=EUC-KR'>"
					+ "</head><body>");
			out.print("</body></html>");
			...
			if(this.SaveOrder(product_num, price, phone, order_comment, buyer)){
				...
				out.print("<html><head>"
						+ "<script>"
						+ "alert('purchase');parent.location.replace('/buyer/test_showproduct');"
						+ "</script>"
						+ "<meta http-equiv='Content-Type' content='text/html; charset=EUC-KR'>"
						+ "</head><body>");
				out.print("</body></html>");
